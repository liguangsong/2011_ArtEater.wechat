	...
	<view class="details" :class='{"details-scroll": timetable}' :style='{height: timetable ? "100vh":"auto"}'>
		...
		<view class="timetable" :class='{"show-timetable": timetable}'>
			<view class="content">
				...
				<view class="list">
					<view class="item" v-for='(item,i) in timetableList' :key='i'
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			// 如果是分享打开的，则查看有没有授权，是不是会员
			var userInfo = uni.getStorageSync('userInfo');
			if (!userInfo) {
				let res = await Curriculum.getCurriculum(options.objectId);
				let info = res[0];
				if (!info.vip) {
					this.curriculumInfo = {};
		...
		async created() {
			...
			uni.getSystemInfo({
				success: (e) => {
					...
					that.tabbarheight = customBar * 2;
					that.navbarheight = customBar * 2 - e.statusBarHeight * 2;
