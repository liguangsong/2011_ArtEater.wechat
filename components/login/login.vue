				...
				user.signUp().then((ruser)=> {
					...
					ruser.save().then(async res=>{
						self.Parse.User.logIn(self.openid,self.openid).then(async lres=>{
							...
							uni.getStorage({
								...
								success(ress) {
									...
									query1.first().then(user=>{
										user.set('parentOpenId', ress.data)
										user.save()
									})
									...
									query2.first().then(user=>{
										...
										q.equalTo('openid', ress.data)
										q.first().then(r=>{
											if (r) {
												// 修改
												let query3 = new self.Parse.Query('PullNew');
												query3.equalTo('openid', ress.data)
												query3.first().then(r1=>{
													r1.set('recruits', Array.from(new Set([lres.openid, ...r1.attributes.recruits])))
													r1.save()
												})
											} else {
												let query4 = new self.Parse.Query('MemberList');
												query4.equalTo('openId', ress.data)
												query4.first().then(parent => {
													let member = undefined;
													if (parent && parent.attributes.memberType) {
														member = Number(parent.attributes.memberType)
													}
													// 创建
													let Extend = self.Parse.Object.extend("PullNew");
													let extend = new Extend();
													extend.set('openid', ress.data);
													extend.set('nickName', user.nickName);
													extend.set('phone', user.phone);
													extend.set('buyNum', 0);
													extend.set('isDispose', false);
													if (typeof member == 'number') {
														extend.set('memberType', member+1);
													}
													extend.set('name', user.realname);
													extend.set('recruits',[ress.data])
													extend.save()
												})
											}
										})
							...
							let parentOpenId = uni.getStorageSync('parentOpenId')
							if (parentOpenId) {
								// 根据父级用户的会员类型发送不同的优惠券
								let parentUser = new self.Parse.Query('MemberList')
								parentUser.equalTo('openId', parentOpenId)
								parentUser = await parentUser.first()
								let memberType = parentUser.attributes.memberType
								if (memberType == 0) {
									couponInfo.equalTo('couponRange', 'blackGoldPullNewUser')
								}
								if (memberType == 1) {
									couponInfo.equalTo('couponRange', 'platinumPullNewUser')
								}
								if (memberType == 2) {
									couponInfo.equalTo('couponRange', 'silverPullNewUser')
								}
								if (memberType == undefined) {
									ouponInfo.equalTo('couponRange', 'newUser')
								}
							} else {
							...
							couponInfo.first().then(coupon=>{
								...
								if (parentOpenId) {
									...
									couponRecord.set("useEndTime", new Date(item.termValidity*1000*60*60*24 + Date.now()))
